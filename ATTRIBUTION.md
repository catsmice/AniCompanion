...
## Character model — Alicia Solid (ニコニ立体ちゃん)
...
- **Notes:** Because the model may not be redistributed, it is **not committed to this repo
  and not bundled in any release**. Each user obtains it themselves (official download, or the
  `scripts/download-model.sh` convenience for their own local use). Full terms in
  `AniCompanion/Resources/VRMModel/LICENSE-AliciaSolid.md`. The project is effectively
  dual-licensed: code MIT, model under Dwango terms.
