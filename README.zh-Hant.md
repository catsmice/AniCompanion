...
- **螢幕視覺**（*選用，預設關閉*）——讓小光看見你目前的視窗（或整個螢幕），對你正在做的事做出反應。
  需要**支援視覺的模型** ＋ 螢幕錄製權限。
- **即時字幕**（*選用，預設關閉*）——小光即時為你 Mac 上正在播放的聲音（影片、會議）加上字幕，還能在你
  觀看時於裝置端**翻譯**（例如日文／韓文 → 中文）。僅顯示文字。見 [即時字幕](docs/live-captions.md)。
...
## v0.6.0 有什麼新功能——即時字幕與翻譯

小光現在能為你 Mac 上正在播放的聲音——一段日文影片、一場會議、一集 podcast——**即時加上字幕**，並可在
你觀看時**翻譯**。她聆聽的是系統音訊，而非你的麥克風。

- **📺 系統音訊即時字幕。** 她以 Apple 裝置端語音引擎轉錄正在播放的內容，顯示在角色下方或桌寵模式的
  對話泡泡中。僅顯示文字。
- **🌏 裝置端翻譯**（*選用*）——在字幕出現時即時翻譯（例如日文／韓文 → 繁體中文），使用 Apple 裝置端
  翻譯，或透過你的 agent 後端 **LLM** 取得更理解前後文的品質。
- **👀 一起看**——字幕執行時，你可以直接問小光正在播放的內容，她會根據最近的逐字稿回答。

預設關閉；需要 macOS 螢幕錄製權限。使用預設值時完全在裝置端運作。見 [即時字幕](docs/live-captions.md)。
...
- **macOS 15.0+**、Apple 晶片即可執行——*即時字幕與裝置端翻譯在 macOS 26 效果最佳*（在 macOS 15 上，
  部分語言會改用 Apple 語音伺服器，且裝置端翻譯無法使用；見 [即時字幕](docs/live-captions.md)）
- **Xcode 26** 才能建置（Swift 6 工具鏈）——即時字幕相關 API 是以 macOS 26 SDK 編譯
